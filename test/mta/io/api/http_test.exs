defmodule MTA.IO.API.HTTP.Test do
  ...
  describe "get_feed_message" do
    ...
    test "calls returns decoded response" do
      ...
      Req.Test.stub(
        MTA.IO.API.HTTP,
        &Plug.Conn.send_resp(&1, 200, IO.iodata_to_binary(protobuf_binary))
      )
    ...
    test "recovers on successful retry" do
      ...
      Req.Test.expect(
        MTA.IO.API.HTTP,
        1,
        &Plug.Conn.send_resp(&1, 200, protobuf_binary)
      )
    ...
    test "retries 3 times" do
      ...
      Req.Test.expect(
        MTA.IO.API.HTTP,
        4,
        &Plug.Conn.send_resp(&1, code, "Internal Server Error")
      )

      assert_raise MTA.IO.API.HTTP.FeedMessageError,
                   "Bad response code: HTTP #{code}",
                   fn ->
    ...
    test "logs when error returned from req" do
      ...
      assert_raise MTA.IO.API.HTTP.FeedMessageError,
                   "Request failed: %Req.TransportError{reason: :#{reason}}",
                   fn ->
                     HTTP.get_feed_message()
                   end
    end
